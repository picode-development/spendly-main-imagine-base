...
// Adds a PopupActivity that handles the spendlywidgets:// scheme -
// launched from a widget button, it opens the voice/search screen as its
// own normal full-screen page (excluded from recents, its own task) so a
// tap on a widget button feels like a quick action, not "open the whole
// app". Previously this used a translucent/transparent window so the
// screen would float over the launcher - real devices render that
// unreliably (Activities can't be truly transparent), showing a dark
// tinge behind the content instead of the launcher, so it's a normal
// opaque full-screen activity now.
function withPopupActivity(config) {
    ...
    config = withDangerousMod(config, [
        ...
        (mod) => {
            ...
            // Normal opaque full-screen theme, inheriting AppTheme's real
            // background — just without a title bar, since the React
